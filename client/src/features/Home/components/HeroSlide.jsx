...
export const HeroSlide = ({ mediaType, mediaCategory }) => {
  ...
  } = useGetCategoriesMoviesQuery({ mediaType });
            ...
            <SwiperSlide key={index}>
              <div className="w-full bg-white dark:bg-black h-[85vh] ">
                ...
                <div className="absolute z-50 top-[30%]  left-[7.5%] ">
                  ...
                  <div className="w-96 flex flex-row mt-2 justify-between">
                        ...
                        .map((genreId, index) => {
                          return (
                            <div
                              ...
                              key={index}
                            >
                              {arrayCategories?.find(
                                (categorie) => categorie.id === genreId
                              ) &&
                                arrayCategories?.find(
                                  (categorie) => categorie.id === genreId
                                ).name}
                  ...
                  <button className="uppercase text-black dark:text-white bg-green-500 py-2 px-4 rounded-md mt-6">
                    <Link to={`media/${mediaType}/${movie.id}`}>Watch Now</Link>
                  </button>
                </div>
              </div>
            </SwiperSlide>
