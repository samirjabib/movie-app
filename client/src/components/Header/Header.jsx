...
export const Header = ({ auth }) => {
  ...
  return (
    <nav
      className={`flex z-50 justify-between p-4 fixed top-0 w-full ${
        backgroundScroll ? "bg-transparent" : "bg-white dark:bg-[#131313]"
      }`}
      onChange={backgroundHandle}
    >
      <div className="flex gap-8">
        <h1 className="font-bold text-2xl self-center  dark:text-white">
          Oni<span className="text-green-500">Giri</span>
        </h1>
      ...
      <button className="bg-green-500 text-sm shadow uppercase rounded-md font-medium dark:text-white px-4 py-2 hidden md:block">
        Sign In
      </button>

      <span className="self-center md:hidden cursor-pointer">
        <AiOutlineMenu size={22} color="white" onClick={onHandleNav} />
      </span>
    </nav>
  );
};
